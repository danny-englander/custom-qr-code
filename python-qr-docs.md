...
A standard install uses [pypng][pypng] to generate PNG files and can also render QR codes directly to the console. A standard install is just:

```bash
pip install qrcode
```

For more image functionality, install qrcode with the `pil` dependency so that [pillow][pillow] is installed and can be used for generating images:
...
Works only with [versions][versions] >=7.2 (SVG styled images require 7.4).

[versions]: https://github.com/lincolnloop/python-qrcode/blob/master/CHANGES.rst#72-19-july-2021
...
![Module drawers](doc/module_drawers.png)
...
![Color masks](doc/color_masks.png)
